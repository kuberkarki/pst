...
class AuthController extends JoshController
{
    ...
    { 
        ...
        try {
            ...
            {
                if(Sentinel::inRole('invoice'))
                     return Redirect::to('admin/order/quickinvoice')->with('success', Lang::get('auth/message.signin.success'));
                elseif(Sentinel::inRole('delivery'))
                    return Redirect::to('admin/order')->with('success', Lang::get('auth/message.signin.success'));
    ...
    public function postForgotPassword(Request $request)
    {
        ...
        try {
            ...
            $message=$settings->content_for_password_reset_mail__c;

            $message=str_replace('%name',$user->first_name.' '.$user->last_name,$message);
            $message=str_replace('%psswd_link',URL::route('forgot-password-confirm', [$user->id, $reminder->code]),$message);
            $message=str_replace('%Store_name',$settings->store_name__c,$message);
